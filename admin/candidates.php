...
$(function(){
  ...
  $('#confirmDeleteAll').click(function(e){
    ...
    $.ajax({
      ...
      error: function(){
        alert('An error occurred while trying to delete all candidates.');
      }
    });
  });

  // Show the "Redo All" modal
  $('#redo_all').click(function(e){
    e.preventDefault();
    $('#redoAllModal').modal('show');
  });

  // Confirm "Redo All" action
  $('#confirmRedoAll').click(function(e){
    e.preventDefault();
    $.ajax({
      type: 'POST',
      url: 'redo_all_candidates.php',
